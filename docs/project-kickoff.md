...
> **当前实现状态(Phase 3.0)**:模式界面(分 short/article 的列表页)
> 尚未实现,所有课程暂时显示在统一的 CoursesPage 列表里;mode 字段
> 在 CourseEditorModal 里通过 radio 选项临时承担选择职责。
>
> 模式界面实现后会:
> 1. CoursesPage 拆成 short / article 两个独立路由,各自只显示
>    对应 mode 的课程
> 2. 模式界面的"新建"按钮直接 preset mode,打开的 Modal 不再有
>    mode radio
> 3. 编辑已有课程时,Modal 显示 mode 但只读(同一课程在 short 模式
>    创建就属于 short,不允许改)
>
> 这是 Phase 3.0 的有意识技术债,优先级低于注释功能;模式界面工程
> 单独排期,不在 Phase 3 范围内。
...
容器(模式) = 用户主动选择,长度 = server-side 校验

**模式的产品语义(用户视角)**:

- **简短模式 (Short)**: 适合格言、短诗、单独的句子或段落,
  从一段完整文本里"摘出来"的片段。强调"快速完成 + 反复练习"。
  字符范围: 5 - 500

- **文章模式 (Article)**: 适合完整的演讲、诗、文章、独立成篇的段落,
  有头有尾、上下文完整的文本。强调"持续练习 + 累积进步"。
  字符范围: 200 - 5000

**Overlap (200-500 字符)**: 在这个区间内,文本既可归 Short 也可归 Article。
这是有意设计的——用户的心理分类基于"上下文是否完整、是否独立成篇",
而非纯字符数。同一段 350 字符的文字,作为某演讲的开头是 Article,
作为单独引用的格言是 Short。系统不强迫用户在边界附近做技术性选择。

**技术下限/上限**:
- 5: 阻挡"乱按几下"的非内容输入
- 5000: Article 上限,前端渲染性能保护
...
---
## Capability Roadmap (project-level)

EchoType 的完整能力清单 + 实现顺序。粒度 = capability(一个能力,不是 sub-step)。
已完成的 capability 保持一行,内部 phase 历史去 git / DECISIONS。
只有 ACTIVE capability 才在 STATE.md 的 Phase Roadmap 里展开。
...
**顺序原则**:从上到下 = 当前执行顺序(基于现状的理解)。顺序会因真实使用
反馈调整,但永远要给出 order——不留空、不写"待定"。理由:对还没碰、
还不知道真实边界的功能,提前细分只会得到一个开发时必然推翻的假规划
(annotation 能干净切成 Phase 1-4,是因为它做完了;边界是回头看才清晰的)。
...
### Shipped
- **Annotation feature** — 注释功能完整闭环:标注(选锚 + overlap 校验)、
  渲染(measurement-driven overlay、跨行、折叠注释槽)、编辑(4 步编辑器)、
  复核(anchoredText 黄/绿比对 + 重锚/删除)。Phase 1–4 done。
- **Cloud deploy** — CloudFront cutover 完成,产品通过默认 `*.cloudfront.net`
  URL 公网可访问;EC2 80 端口锁到 CloudFront managed prefix list,
  S3 私桶通过 OAC 走;GitHub Actions OIDC + SSM Run Command 部署链路打通。
  当前处于 self-testing 阶段(不在任何 active capability 内)。


### Next — execution order from current understanding
- **Course management** — short / article 独立路由、card 列表、搜索 / 过滤 / 
  排序、增删入口;移除编辑器里的临时 mode selector(Phase 3.0 技术债)。
- **Typing experience** — 基础打字 + 红绿实时对错已有(walking skeleton);
  待打磨:IME 输入处理、session 流程(循环 / 暂停 / 结束)。
- **Course stats** — per-session + 累积统计。
  > 依赖 typing session 数据,故客观上排在 typing 打磨之后。
- **Auth** — Cognito(Google + email/password,email-based upsert),
  替换 demo-user shim。
  > 自己用 demo-user 够了,可延后;**分享给外部用户(如闲鱼)前必须**。
- **Custom domain** — 购买域名 + ACM 证书(us-east-1) + CloudFront alias
  配置;Route 53 或外部 DNS 接入。
  > 默认 `*.cloudfront.net` URL 自己用够,故延后;**真正分享前再做**。
- **Ops & safety** — Sentry、CloudWatch、rate limiting、免责声明 / 
  disclaimer、错误页 / 空状态。
  > 这一层是"给别人用之前"的就绪项,零散且非功能性,不构成一个有清晰
  > phase 的 capability;做到时按需补,不提前细分。
